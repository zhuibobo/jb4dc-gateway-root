...
REDIPS.drag = (function () {
        ...
        var init,						// initialization
            ...
            only = {
                div: {},				// (object) DIVid -> classname; defined DIV elements can be placed only to the 'redips-only' TD cells (TD should be marked with special className)
            ...
            mark = {
                action: 'deny',			// (string) default action form marked TD is deny
                cname: 'redips-mark',		// (string) class name for marked cells
                exception: {},				// (object) DIV id exception for marked cell
                exceptionClass: {}
            },		// (object) DIV class name exception for marked cell
            style = {
        ...
        let __delay = 200;
        let __timeObject;
        let coverHandlerOnMouseDown = function (e) {
            __click++;
            let _this = this;
            if (__click === 1) {
                __timeObject = setTimeout(function () {
                    console.log("coverHandlerOnMouseDowncoverHandlerOnMouseDowncoverHandlerOnMouseDown")
                    handlerOnMouseDown.call(_this, e);
                    __click = 0;
                }, __delay);
                return false
            } else {
                __click = 0;
                clearTimeout(__timeObject);
            }
        };*/
        /**
        ...
        insertContent = function (targetTable, param) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                if (targetTable.rows[r] === undefined) {
                    ...
                    flag = REDIPS.drag.error.loadContent({
                        type: 1,
                        message: 'Target TR [' + r + '] does not exist',
                        content: divContent,
                        rowIndex: r,
                        cellIndex: c
                    });
                ...
                // if target cell doesn't exist then set flag to false
                    ...
                    flag = REDIPS.drag.error.loadContent({
                        type: 2,
                        message: 'Target TD [' + r + ',' + c + '] does not exist',
                        content: divContent,
                        rowIndex: r,
                        cellIndex: c
                    });
